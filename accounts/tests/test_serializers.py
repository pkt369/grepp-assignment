...
class UserSerializerTest:
    ...
    def test_duplicate_email(self):
        ...
        assert serializer.errors['email'][0] == '이미 사용 중인 이메일입니다.'
    ...
    def test_password_no_letter(self):
        ...
        assert not serializer.is_valid()
        assert 'password' in serializer.errors
        assert serializer.errors['password'][0] == '비밀번호는 최소 하나의 문자를 포함해야 합니다.'
    ...
    def test_password_no_number(self):
        ...
        assert not serializer.is_valid()
        assert 'password' in serializer.errors
        assert serializer.errors['password'][0] == '비밀번호는 최소 하나의 숫자를 포함해야 합니다.'
...
@pytest.mark.django_db
class CustomTokenObtainPairSerializerTest:
    ...
    def test_token_contains_custom_claims(self):
        ...
        # 커스텀 클레임 확인
        assert custom_token['email'] == self.user.email
        assert custom_token['username'] == self.user.username
