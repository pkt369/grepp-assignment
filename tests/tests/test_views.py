...
class TestTestViewSet:
    ...
    def test_list_tests_is_registered_field(self):
        ...
        assert results[self.test1.id]['is_registered']
        assert results[self.test2.id]['is_registered']
        assert not results[self.test3.id]['is_registered']
    ...
    def test_list_tests_registration_count_field(self):
        ...
        assert results[self.test1.id]['registration_count'] == 2
        assert results[self.test2.id]['registration_count'] == 1
        assert results[self.test3.id]['registration_count'] == 0
    ...
    def test_list_tests_different_user_sees_different_is_registered(self):
        ...
        # test1: 둘 다 등록
        assert results1[self.test1.id]['is_registered']
        assert results2[self.test1.id]['is_registered']

        # test2: user만 등록
        assert results1[self.test2.id]['is_registered']
        assert not results2[self.test2.id]['is_registered']
    ...
    def test_retrieve_test_authenticated(self):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data['id'] == self.test1.id
        assert response.data['title'] == self.test1.title
    ...
    def test_filter_by_status_available(self):
        ...
        assert self.test1.id in ids
        assert self.test2.id in ids
        assert self.test3.id not in ids
    ...
    def test_search_by_keyword(self):
        ...
        assert response.data['results'][0]['id'] == self.test1.id
    ...
    def test_sort_by_created(self):
        ...
        assert results[0]['id'] == self.test3.id
        assert results[1]['id'] == self.test2.id
        assert results[2]['id'] == self.test1.id
    ...
    def test_sort_by_popular(self):
        ...
        assert results[0]['id'] == self.test1.id
        assert results[0]['registration_count'] == 2
        assert results[1]['id'] == self.test2.id
        assert results[1]['registration_count'] == 1
        assert results[2]['id'] == self.test3.id
        assert results[2]['registration_count'] == 0
